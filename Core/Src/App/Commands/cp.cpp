...
CmdExec cp_exec = [](const etl::vector<estring, ARGS_CAPACITY>& args)
{
    ...
    // if (!sd_reader.exists(src.c_str())) return SD_RES::ERR;

    // if (sd_reader.is_directory(dst.c_str()))
    // {
    //     bool is_src_dir = sd_reader.is_directory(src.c_str());
    ...
    // else
    // {
    //     if (sd_reader.is_directory(src.c_str()))
